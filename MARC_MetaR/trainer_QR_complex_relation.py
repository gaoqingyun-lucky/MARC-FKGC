from models_QR_complex_relation import *
...
class Trainer:
    ...
    def do_one_step(self, task, iseval=False, curr_rel=''):
        loss, p_score, n_score, latent_orthogonal_loss = 0, 0, 0, 0
        if not iseval:
            self.optimizer.zero_grad()
            p_score, n_score, latent_orthogonal_loss = self.metaR(task, iseval, curr_rel)
            y = torch.Tensor([1]).to(self.device)
            y = y.unsqueeze(0).expand(p_score.shape)
            loss = self.metaR.loss_func(p_score, n_score, y) + 0.01 * latent_orthogonal_loss
            loss.backward()
            self.optimizer.step()
        elif curr_rel != '':
            p_score, n_score, latent_orthogonal_loss = self.metaR(task, iseval, curr_rel)
            ...
            y = y.unsqueeze(0).expand(p_score.shape)
            loss = self.metaR.loss_func(p_score, n_score, y) + 0.01 * latent_orthogonal_loss
        return loss, p_score, n_score, latent_orthogonal_loss
    ...
    def train(self):
        ...
        for e in range(self.epoch):
            ...
            loss, _, _, latent_orthogonal_loss = self.do_one_step(train_task, iseval=False, curr_rel=curr_rel)
            # print the loss on specific epoch
            if e % self.print_epoch == 0:
                loss_num = loss.item()
                self.write_training_log({'Loss': loss_num}, e)
                print("Epoch: {}\tLoss: {:.4f}\tOrLoss: {:.4f}".format(e, loss_num,latent_orthogonal_loss.item()))
    ...
    def eval(self, istest=False, epoch=None):
        ...
        while True:
            ...
            _, p_score, n_score, _ = self.do_one_step(eval_task, iseval=True, curr_rel=curr_rel)
    ...
    def eval_by_relation(self, istest=False, epoch=None):
        ...
        for rel in data_loader.all_rels:
            ...
            while True:
                ...
                _, p_score, n_score, _ = self.do_one_step(eval_task, iseval=True, curr_rel=rel)
                x = torch.cat([n_score, p_score], 1).squeeze()
